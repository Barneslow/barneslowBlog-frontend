...
const PostDetails = () => {
  ...
  const { postDetails, loading, appError, serverError, isDeleted } = post;

  const { createdComment, commentDeleted } = allComment;

  useEffect(() => {
    dispatch(fetchPostDetailsAction(id));
  }, [id, dispatch, createdComment, commentDeleted]);
  ...
  return (
    <>
      {loading ? (
        ...
        <h2 className="h-screen text-red-400 text-xl">
          {appError} {serverError}
        </h2>
      ) : postDetails === null ? (
        <h1>No Post Found</h1>
