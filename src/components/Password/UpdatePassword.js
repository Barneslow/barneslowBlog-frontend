...
import { updatePasswordAction } from "../../store/passwordSlice";
...
const UpdatePassword = () => {
  ...
  const { isPasswordUpdated, loading, appError, serverError } = password;
  ...
  return (
    <div className="min-h-screen bg-gray-700  flex flex-col justify-center py-12 sm:px-6 lg:px-8">
      <div className="sm:mx-auto sm:w-full sm:max-w-md">
        ...
        {appError ||
          (serverError && (
            <h3 className="text-center text-red-400 pt-2">
          ...
          <form onSubmit={formik.handleSubmit} className="space-y-6">
            ...
            <div>
              ...
              {loading ? (
                <button
                  disabled
                  className="inline-flex bg-indigo-700 justify-center w-full px-4 py-2 border border-gray-300 shadow-sm text-sm font-medium rounded-md text-gray-200"
                >
                  <span>Loading...</span>
                </button>
              ) : (
